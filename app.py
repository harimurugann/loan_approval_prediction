...
def log_user_data(name, income, credit, amount, result, prob):
    ...
    # Adding Dummy Geo Data for Mapping (Chennai)
...
# --- NAVIGATION TABS (All 8 Features) ---
tabs = st.tabs([
...
with tabs[0]:
    ...
    if st.button("Run AI Prediction"):
        # 24 Column Formatting for Pipeline
...
with tabs[2]:
    ...
    if os.path.exists('user_logs_production.csv'):
        ...
        # Check if lat/lon exists and show map
        if 'lat' in df_geo.columns:
            st.map(df_geo[['lat', 'lon']])
            st.info("📍 Showing live locations of simulated loan applicants.")
    else:
        st.warning("⚠️ No data available. Please run an assessment first.")

# --- TAB 7: FUTURE ROADMAP (INDENTATION FIXED) ---
with tabs[7]:
    st.header("🚀 Future Roadmap")
    st.write("Developing: NLP Sentiment Analysis for Loan Applicants.")
    st.write("Upcoming: Integration with decentralized finance (DeFi) protocols.")
