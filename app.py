...
def generate_pro_pdf(df, report_title="Audit Log"):
    ...
    for _, row in df.iterrows():
        ...
        pdf.cell(50, 10, str(row['Applicant_Name'])[:20], 1)
...
def log_user_data(name, income, credit, amount, result, prob):
    ...
    if not os.path.isfile(file):
        log_entry.to_csv(file, index=False)
    else:
        log_entry.to_csv(file, mode='a', header=False, index=False)
...
with tabs[1]:
    ...
    if up:
        st.dataframe(pd.read_csv(up).head())
...
with tabs[2]:
    ...
    if os.path.exists('user_logs_pro.csv'):
        ...
        st.info("No data yet. Run an assessment.")
...
with tabs[6]:
    ...
    if pwd == "admin123":
        if os.path.exists('user_logs_pro.csv'):
            ...
            if search:
                filtered = full_df[full_df['Applicant_Name'].str.contains(search, case=False, na=False)]
            else:
                filtered = full_df
            
            # 2. Multi-Select Logic
            st.subheader("Selection & Batch Export")
            selected = st.multiselect("Select Clients for Batch Download", options=filtered['Applicant_Name'].unique())
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.download_button("📥 Download Selection (CSV)", final_df.to_csv(index=False), "Bank_Logs.csv", "text/csv")
            with col2:
                pdf_bytes = generate_pro_pdf(final_df, report_title="Batch Audit Report")
